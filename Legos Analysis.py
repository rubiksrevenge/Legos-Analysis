...
num_colors = len(colors['name'].value_counts())
...
#colors_summary = colors.nditer(colors['is_trans']).count(colors_summary)
...
parts_by_year = sets.groupby('year').agg('median')
...
themes_by_year = sets.groupby('year')['theme_id'].agg('sum')
